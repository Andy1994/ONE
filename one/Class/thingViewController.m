...
@interface thingViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
- (void)dowload{
    ...
    for (int i = 1; i <=10; i++) {
        NSString * urlString=[NSString stringWithFormat:@"http://bea.wufazhuce.com/OneForWeb/one/o_f?strDate=%@&strRow=%d",dateStr2,i];
        AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
        manager.responseSerializer = [AFHTTPResponseSerializer serializer];
        [manager GET:urlString parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            
            NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
            NSDictionary *apps = dict[@"entTg"];
            _model = [[thingModel alloc] init];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    thingCollectionViewCell * cell = [thingCollectionViewCell cellWithTableView:collectionView andIndexPath:indexPath];
